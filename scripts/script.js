...
function toUpper() {
  let text = document.getElementById("lowerCase").value;
  let output = document.getElementById("outputUpper");

  if (text[0] === text[0].toUpperCase()) {
    output.innerHTML = "the word starts with upper case";
  } else {
    let textver1 = text;
    let letter = text[0].toUpperCase();
    text = letter + text.slice(1);
    output.innerHTML = `The text changed from '${textver1}' to '${text}'`;
  }
}
